...
class SavePlugin(Plugin):
    ...
    # 插件名称
    name: str = "Save Plugin"

    # 插件描述
    description: str = "负责保存抓取的页面到磁盘的插件"
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.static_resources: Optional[Set[str]] = None
        self.saved_files: List[Tuple[str, str]] = []
        self.downloaded_resources: Set[str] = set()  # 已下载的资源集合
        self.downloader_lock: threading.Lock = threading.Lock()  # 下载器锁
        self.target_directory: str = "/"

        # 资源下载队列和线程控制
        self.resource_queue: queue.Queue[str] = queue.Queue()
        self.resource_thread: Optional[threading.Thread] = None
        self.resource_thread_stop: threading.Event = threading.Event()
    ...
    def on_save_start(self, saver_data: Dict[str, Any]) -> None:
        ...
        # 获取静态资源集合，并规范化所有 URL
        raw_static_resources: Set[str] = saver_data.get('static_resources', set())
    ...
    def save_site(self, pages: Dict[str, str]) -> List[Tuple[str, str]]:
        ...
        processed_pages: Dict[str, str] = self._process_all_links(pages)

        # 将处理后的页面保存到磁盘
        self.logger.info(_t("开始保存页面到磁盘，共") + f" {len(processed_pages)} " + _t("个页面"))
        saved_count: int = self._save_pages(processed_pages)
    ...
    def _process_all_links(self, pages: Dict[str, str]) -> Dict[str, str]:
        """统一处理所有页面的链接
        ...
        Returns:
            处理后的页面内容，键为URL，值为处理后的HTML内容
        """
        processed_pages: Dict[str, str] = {}

        # 记录静态资源统计信息
        self.logger.info(_t("待处理静态资源数量") + f": {len(self.static_resources)}")
        queued_resources: Set[str] = set()  # 跟踪已加入队列的资源
        ...
        for url, html_content in pages.items():
            try:
                ...
                local_file_path: str = self._get_local_file_path(url)
                # 获取相对于输出目录的相对路径
                rel_local_path: str = os.path.relpath(local_file_path, self.output_dir)
                # 转换为URL风格的路径（使用/分隔符）
                rel_local_path = rel_local_path.replace('\\', '/')
                # 如果路径以 index.html 或 index.htm 结尾，将其视为目录
                base_path: str = rel_local_path
                ...
                for link in all_links:
                    ...
                    if link.name == 'a':
                        ...
                        if href:
                            ...
                            if full_url in pages:
                                # 转换为相对于当前页面的本地路径
                                local_path: str = self._url_to_local_path(full_url, base_path)
                    ...
                    # 处理img、link、script标签的src或href
                        ...
                        src: Optional[str] = link.get('src') or link.get('href')
                        ...
                        if src:
                            ...
                            if full_url in self.static_resources:
                                ...
                                # 构建静态资源的本地路径
                                static_local_path: str = self._get_local_file_path(full_url)
                                # 获取当前页面的本地文件路径
                                current_local_path: str = self._get_local_file_path(url)
                                # 获取当前页面所在的目录
                                current_dir: str = os.path.dirname(current_local_path)
                                # 计算从当前页面目录到静态资源的相对路径
                                relative_path: str = os.path.relpath(static_local_path, current_dir)
                                # 转换为URL风格的路径（使用/分隔符）
    ...
    def _save_pages(self, processed_pages: Dict[str, str]) -> int:
        """将处理后的页面保存到磁盘
        ...
        for url, html_content in processed_pages.items():
            try:
                # 获取文件路径
                file_path: str = self._get_file_path(url)
    ...
    def _is_same_domain(self, url: str) -> bool:
        """检查是否为同域名"""
        target_domain: str = urlparse(self.target_url).netloc
        current_domain: str = urlparse(url).netloc
    ...
    def _get_local_file_path(self, url: str) -> str:
        """根据URL获取本地文件保存路径
    ...
    def _url_to_local_path(self, url: str, base_path: str) -> str:
        """将URL转换为本地路径
        ...
        # 移除开头的/，得到绝对路径
        absolute_path: str = path.lstrip('/')

        # 从base_path中提取当前目录的层级
        base_parts: List[str] = [part for part in base_path.lstrip('/').split('/') if part]

        # 移除absolute_path开头的与base_path相同的部分
        absolute_parts: List[str] = absolute_path.split('/')

        # 找到相同的前缀
        common_prefix: List[str] = []
        for base_part, abs_part in zip(base_parts, absolute_parts):
        ...
        # 计算相对路径
        # 向上返回的目录数
        up_count: int = len(base_parts) - len(common_prefix)
        # 向下的路径部分
        down_parts: List[str] = absolute_parts[len(common_prefix):]

        # 构建相对路径
        relative_parts: List[str] = ['..'] * up_count + down_parts
        relative_path: str = '/'.join(relative_parts)
    ...
    def _start_resource_thread(self) -> None:
        """启动资源下载线程"""
    ...
    def _stop_resource_thread(self) -> None:
        """停止资源下载线程"""
    ...
    def _resource_worker(self) -> None:
        ...
        while not self.resource_thread_stop.is_set():
            ...
            try:
                ...
                file_path: Optional[str] = self.on_download_resource(static_url, self.output_dir)
